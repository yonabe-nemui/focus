...
| 28 | Android predictive back 未対応 | UX | 低 | 中 | 見送り |
| 29 | ライセンス表示(計画書一覧 + アプリ内画面) | 追加 | 低 | 低 | P5 ✅ |
...
## フェーズ3: UX・規約まわり【対応済み 2026-07-17】

> 3-3 は P2-1 のエラー型導入で大半が完了済みだったため、残っていた
> `LOGIN_FAILED` の種別化と英語フォールバック文字列の排除、
> 文言解決の `feedErrorMessage()`(composeApp)への一元化のみ実施。
> 3-4 は URL 未設定(空文字列)時に `serverBaseUrl = null` として
> 各ソースへ直接アクセスするフォールバックを追加した。
...
## フェーズ5: デザイン・UX モダナイズ【対応済み 2026-07-18】
...
> 実施メモ(2026-07-18):
> - 5-8 は UserPreferenceEntity + PreferenceStore(Web はメモリ保持)で永続化し、
>   ダイナミックカラーは expect/actual で Android 12+ のみ有効化した。
> - 5-9 は3ウェイト構成(Regular / Medium / Bold)。Noto Sans JP は JP サブセット OTF を使用。
>   OFL ライセンス文は composeResources/files/licenses/ に同梱。
> - 5-12 のリフレッシュショートカットは、r 単独だと検索欄の入力と衝突するため Ctrl+R とした。
> - ユーザー指示によりライセンス表示(5-13)を追加実施した。
> - 見送り: 「新着 N 件」ピル(任意項目)、デバウンス検索(BlueSky レート制限リスク)、
>   j/k 記事移動(マルチカラムで対象カラムが曖昧)、predictive back / Navigation 3 移行(影響範囲大)。
...
- ユーザー指示(2026-07-18)によりフェーズ5に追加。
- 本計画書に「利用ライブラリ・フォントとライセンス」一覧を記載し、
  アプリ内にも設定画面から開ける `LicensesScreen`(名称・ライセンス・URL のリスト、
  タップでブラウザ表示)を実装した。
